...
function createRasterStyle(
  theme: ResolvedTheme,
  provider: (typeof TILE_PROVIDERS)[number],
): StyleSpecification {
  return {
    version: 8,
    sources: {
      raster: {
        type: "raster",
        tiles: [provider.url.replace("{z}", "{z}").replace("{x}", "{x}").replace("{y}", "{y}")],
        tileSize: 256,
...
export function generateMapStyle(
  ...
  const tileProviderId = options?.tileProvider || "openfreemap";
  const tileProvider = TILE_PROVIDERS.find((t) => t.id === tileProviderId);
  if (tileProvider && tileProvider.type !== "vector") {
    return createRasterStyle(theme, tileProvider);
  }
  const buildingFill =
    theme.map.buildings ||
  ...
  const focusCountry = options?.focusCountry ?? false;
  ...
  const focusVisibility = focusCountry ? ("visible" as const) : ("none" as const);
  const dimColor = blendHex(theme.map.land, "#000000", 0.35);
    ...
    layers: [
      ...
      },
      {
        ...
        paint: {
          ...
          "line-opacity":
            includeRoadOutline && includeRoadPath
              ? opacityExpr([
                  [8, 0.62],
                  [12, 0.72],
                  [18, 0.85],
...

      {
        ...
        layout: { visibility: focusVisibility },
      },
      {
        id: "country-boundary",
        source: SOURCE_ID,
