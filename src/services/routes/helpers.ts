import { haversineMeters } from "@/utils/geo/math";
...
export function routeEndpoints(route: Route): { start: Coordinate; finish: Coordinate } | null {
...
export function boundsHalfWidthMeters(bounds: RouteBounds): number {
  ...
  const lonMeters = lonSpan * METERS_PER_DEGREE_LAT * Math.cos((center.lat * Math.PI) / 180);
