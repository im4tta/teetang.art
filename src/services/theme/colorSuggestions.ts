import { normalizeHexColor, shiftHexColor, toUniqueHexColors } from "@/utils/color";
import type { ThemeOption, ResolvedTheme } from "@/services/theme/types";
...
export function createFallbackThemeOption(
  ...
  getThemePalette: (theme: unknown) => string[] = defaultGetThemePalette,
): ThemeOption {
  return {
    id: themeId,
...
export function buildDynamicColorChoices(
  ...
  function addMore(color: string) {
    const normalized = normalizeHexColor(color);
    if (!normalized || suggestedSeen.has(normalized) || moreSeen.has(normalized)) return;
