...
export const predefinedMarkerIcons: MarkerIconDefinition[] = [
  createImageIcon("app-marker", "Tee Tang Art", "/assets/marker.svg"),
...
export const featuredMarkerIcons = predefinedMarkerIcons.slice(0, MARKER_FEATURED_ICON_COUNT);

export function getAllMarkerIcons(customIcons: MarkerIconDefinition[]) {
  return [...predefinedMarkerIcons, ...customIcons];
}

export function findMarkerIcon(iconId: string, customIcons: MarkerIconDefinition[]) {
  return getAllMarkerIcons(customIcons).find((icon) => icon.id === iconId) ?? null;
}
