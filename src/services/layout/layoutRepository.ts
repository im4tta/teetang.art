...
import type { Layout, LayoutGroup } from "@/services/layout/types";
...
export const layoutOptions: Layout[] = layoutGroups.flatMap((group) => group.options);
...
const manifestDefaultId = String((layoutsManifest as any)?.defaultLayoutId ?? "").trim();
...
export function getLayoutOption(layoutId: string): Layout | null {
  ...
  if (layoutOptionsById[defaultLayoutId]) return layoutOptionsById[defaultLayoutId];
  return fallbackLayout;
}

export function createCustomLayoutOption(widthCm: number, heightCm: number): Layout {
