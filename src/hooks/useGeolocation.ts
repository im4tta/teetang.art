...
/** Centres a fresh poster on the visitor's position; skipped when `enabled` is false. */
export function useGeolocation(dispatch: React.Dispatch<PosterAction>, enabled: boolean) {
  useEffect(() => {
    if (!enabled) return;
    let cancelled = false;
    const fallback = () => {
