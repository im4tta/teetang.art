...
export function distanceToZoom(
  ...
  return clamp(
    Math.log2((EARTH_CIRCUMFERENCE_M * cosLat * containerPx) / (distanceMeters * 2 * TILE_SIZE_PX)),
    MIN_MAP_ZOOM,
    MAX_MAP_ZOOM,
  );
}
...
export function zoomToDistance(zoom: number, latDeg: number, containerPx: number): number {
  const cosLat = Math.max(0.01, Math.cos((Math.abs(latDeg) * Math.PI) / 180));
  return clamp(
    Math.round((EARTH_CIRCUMFERENCE_M * cosLat * containerPx) / (2 ** zoom * TILE_SIZE_PX * 2)),
    MIN_DISTANCE_METERS,
    MAX_DISTANCE_METERS,
  );
}
...
export function useMapSync(state: MapSyncState, dispatch: MapSyncDispatch, mapRef: MapInstanceRef) {
  ...
  const mapZoom = useMemo(
    () =>
      clamp(
        distanceToZoom(formDistance, formLat, effectivePx),
        zoomBounds.minZoom,
        zoomBounds.maxZoom,
      ),
  ...
  const handleMoveEnd = useCallback(
    (center: [number, number], zoom: number) => {
      ...
      const distance = zoomToDistance(
        clamp(zoom, bounds.minZoom, bounds.maxZoom),
        lat,
        effectivePx,
      );
  ...
  useEffect(() => {
    ...
    if (!form.latitude || !form.longitude || state.selectedLocation) return;
    if (!Number.isFinite(lat) || !Number.isFinite(lon)) return;
    const key = `${lat.toFixed(6)},${lon.toFixed(6)}`;
    if (skippedRef.current === key) {
      skippedRef.current = "";
      return;
    }
    if (lastManualRef.current === key) return;
    const tid = window.setTimeout(() => {
      lastManualRef.current = key;
      updateFromCoords(lat, lon);
    }, 350);
    return () => window.clearTimeout(tid);
  }, [form.latitude, form.longitude, state.selectedLocation, updateFromCoords]);

  const flyToLocation = useCallback(
    (lat: number, lon: number, keepZoom = false, distanceOverride?: number) => {
      const map = mapRef.current;
      if (!map) return;
      const d = distanceOverride ?? formDistance;
      const bounds = resolveZoomBounds(lat, effectivePx);
      const zoom = keepZoom
        ? clamp(map.getZoom(), bounds.minZoom, bounds.maxZoom)
        : clamp(distanceToZoom(d, lat, effectivePx), bounds.minZoom, bounds.maxZoom);
  ...
  return {
    mapCenter,
    mapZoom,
    mapMinZoom: zoomBounds.minZoom,
    mapMaxZoom: zoomBounds.maxZoom,
    handleMove,
    handleMoveEnd,
    flyToLocation,
    setContainerWidth,
