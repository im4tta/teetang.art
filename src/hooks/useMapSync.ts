...
import { MIN_DISTANCE_METERS, MAX_DISTANCE_METERS, EARTH_CIRCUMFERENCE_M, TILE_SIZE_PX, MIN_MAP_ZOOM, MAX_MAP_ZOOM, DEFAULT_CONTAINER_PX, FLY_TO_DURATION_MS } from "@/services/config";
import { MAP_OVERZOOM_SCALE, MIN_EFFECTIVE_CONTAINER_PX, MAX_OVERZOOM_SCALE } from "@/services/map/constants";
...
export function useMapSync(state: MapSyncState, dispatch: MapSyncDispatch, mapRef: MapInstanceRef) {
  ...
  const flyToLocation = useCallback((lat: number, lon: number, keepZoom = false, distanceOverride?: number) => {
