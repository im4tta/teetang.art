...
import { getLayoutOption, layoutOptions } from "@/services/layout/layoutRepository";
import {
  MIN_POSTER_CM,
  MAX_POSTER_CM,
  MIN_DISTANCE_METERS,
...
/**
 * The part of a change event the form handler reads, so controls can also call
 * it directly with `{ target: { name, value } }`.
 */
export type FormChangeHandler = (event: {
...
export function useFormHandlers() {
    ...
    (event: React.FocusEvent<HTMLInputElement>) => {
      ...
      if (name === "distance") {
        dispatch({
          type: "SET_FIELD",
          name: "distance",
          value: String(Math.round(clamp(parsed, MIN_DISTANCE_METERS, MAX_DISTANCE_METERS))),
        });
        return;
      }
      if (name === "width" || name === "height") {
        ...
        dispatch({
          ...
          fields: {
            width: formatLayoutCm(nW),
            height: formatLayoutCm(nH),
  ...
  const handleThemeChange = useCallback(
    (themeId: string) => dispatch({ type: "SET_THEME", themeId }),
    [dispatch],
  );
  const handleTheme2Change = useCallback(
    (themeId: string) => dispatch({ type: "SET_THEME2", themeId }),
    [dispatch],
  );
    ...
    (layoutId: string) => {
      ...
      if (opt)
        dispatch({
          type: "SET_LAYOUT",
          layoutId: opt.id,
          widthCm: formatLayoutCm(opt.widthCm),
          heightCm: formatLayoutCm(opt.heightCm),
        });
    },
    [dispatch],
  );
  ...
  const handleColorChange = useCallback(
    (key: string, value: string) => dispatch({ type: "SET_COLOR", key, value }),
    [dispatch],
  );
  ...
  const setLocationFocused = useCallback(
    (focused: boolean) => dispatch({ type: "SET_LOCATION_FOCUSED", focused }),
    [dispatch],
  );
  const handleCreditsChange = useCallback(
    (value: boolean) => dispatch({ type: "SET_FIELD", name: "includeCredits", value }),
    [dispatch],
  );
