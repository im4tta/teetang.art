...
export function validatePosterInput(
  ...
  if (!locationText && (latText || lonText) && !hasManualCoordinates) {
    throw new Error("When location is empty, both latitude and longitude are required.");
  }

  if (!locationText && !hasManualCoordinates) {
    throw new Error("Location is required unless both latitude and longitude are provided.");
  }
