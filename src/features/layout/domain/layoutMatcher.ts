...
export function resolveLayoutIdForSize(
  ...
  if (currentLayout && matchesLayoutSize(currentLayout, widthCm, heightCm, toleranceCm)) {
    return currentLayout.id;
  }

  const matching = allLayouts.find((lo) => matchesLayoutSize(lo, widthCm, heightCm, toleranceCm));
