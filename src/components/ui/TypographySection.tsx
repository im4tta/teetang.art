...
import type { FontOption } from "@/services/config";
import {
  ...
  return (
    <label className="toggle-field">
      ...
      <span className="theme-select-wrapper">
        <select className="theme-select" name={name} value={value} onChange={onChange}>
          {children}
        </select>
      </span>
    </label>
    ...
    <>
      <section className="panel-block">
        ...
        <Toggle
          ...
          name="showUnderline"
          checked={Boolean(form.showUnderline)}
          onChange={onChange}
        />

        <div className="field-grid keep-two-mobile">
          <Field
            label={t("poster.displayCity")}
            name="displayCity"
        ...
        <p className="section-summary-label" style={{ marginTop: 12 }}>
          {t("text.footerText")}
        </p>
        <div className="field-grid keep-two-mobile">
          <Field
            label={t("text.footerName")}
            name="footerCity"
          ...
          >
            {fontOptions.map((o) => (
              <option
                ...
                style={{
                  fontFamily: o.value
                    ? `"${o.value}", "Space Grotesk", sans-serif`
      ...
      <section className="panel-block">
        ...
        <div className="shape-picker">
          {POSTER_SHAPES.map((s) => (
            <button
              key={s}
              type="button"
              className={`shape-btn${form.mapShape === s ? " active" : ""}`}
              onClick={() => onChange({ target: { name: "mapShape", value: s } })}
        ...
        {form.layoutMode === "dual-city" ? (
          <p
            ...
            style={{ color: "#94A3B8", textAlign: "center", padding: "12px 0" }}
          >
            {t("dualCity.activeHint")}
          </p>
        ) : (
          <SelectField
            label={t("text.layoutMode")}
            name="layoutMode"
        ...
        {form.layoutMode === "property-card" && (
          <section className="panel-block" style={{ marginTop: 12 }}>
            ...
            <div className="field-grid">
              ...
              />
              <Field
                label={t("property.landSize")}
                name="propLandSize"
                value={form.propLandSize}
                onChange={onChange}
              />
              <Field
                label={t("property.buildSize")}
                name="propBuildSize"
                value={form.propBuildSize}
                onChange={onChange}
              />
              <Field
                label={t("property.bedrooms")}
                name="propBedrooms"
                value={form.propBedrooms}
                onChange={onChange}
                type="number"
              />
              <Field
                ...
                name="propBathrooms"
                value={form.propBathrooms}
                onChange={onChange}
                type="number"
              />
            </div>
            <Field
              label={t("property.features")}
              name="propFeatures"
              value={form.propFeatures}
              onChange={onChange}
            />
            <Field
              label={t("property.contactNumber")}
              name="propContact"
              value={form.propContact}
              onChange={onChange}
            />
            <Field
              label={t("property.agentName")}
              name="propAgent"
              value={form.propAgent}
              onChange={onChange}
            />
            <Field
              label={t("property.ctaText")}
              name="propCta"
              value={form.propCta}
              onChange={onChange}
            />
        ...
        {form.layoutMode === "shop-signage" && (
          <section className="panel-block" style={{ marginTop: 12 }}>
            ...
            <div className="field-grid">
              ...
              />
              <Field
                label={t("shop.telegram")}
                name="shopTelegram"
                value={form.shopTelegram}
                onChange={onChange}
        ...
        <Toggle
          label={t("layout.decorativeBorder")}
          name="showBorder"
          checked={Boolean(form.showBorder)}
          onChange={onChange}
        />
        <Toggle
          label={t("poster.showCoordinates")}
        ...
        {form.showCoordinates && (
          <SelectField
            label={t("coords.format")}
            name="coordsFormat"
            value={form.coordsFormat}
            onChange={onChange}
          >
        ...
        <Toggle
          label={t("poster.allCaps")}
          name="titleAllCaps"
          checked={Boolean(form.titleAllCaps)}
          onChange={onChange}
        />
      </section>
      ...
      <section className="panel-block">
        ...
        />
        <SelectField
          label={t("radius.style")}
          name="radiusStyle"
          value={form.radiusStyle}
          onChange={onChange}
        >
          <option value="dashed">{t("radius.dashed")}</option>
          <option value="filled">{t("radius.filled")}</option>
          <option value="gradient">{t("radius.gradient")}</option>
        </SelectField>
      </section>

      <section className="panel-block">
        <p className="section-summary-label">{t("poi.nearby")}</p>
        <Toggle
          label={t("poi.show")}
          name="showPois"
          checked={Boolean(form.showPois)}
          onChange={onChange}
        />
        {form.showPois && (
          <>
            {(
              [
                ["poiSchools", "poi.schools"],
                ["poiHospitals", "poi.hospitals"],
                ["poiMarkets", "poi.markets"],
                ["poiBanks", "poi.banks"],
                ["poiRestaurants", "poi.restaurants"],
              ] as const
            ).map(([name, key]) => (
              <Toggle
                key={name}
                label={t(key)}
                name={name}
                checked={Boolean(form[name as keyof PosterForm])}
                onChange={onChange}
              />
            ))}
          </>
...
function QRSection({ form, onChange }: { form: PosterForm; onChange: (e: any) => void }) {
  ...
  const setXY = (field: string, v: number) =>
    dispatch({ type: "SET_FORM_FIELDS", fields: { [field]: String(v), qrPosition: "custom" } });

  return (
    <section className="panel-block">
      <p className="section-summary-label">{t("layout.qrCode")}</p>
      <Toggle
        label={t("layout.qrCode")}
        name="showQrCode"
      ...
      {form.showQrCode && (
        <>
          ...
          {(form.qrDestination === "whatsapp" || form.qrDestination === "telegram") && (
            <Field
              label={t("qr.phoneUsername")}
              name="qrPhone"
              value={form.qrPhone}
              onChange={onChange}
              placeholder={form.qrDestination === "whatsapp" ? "+855..." : "@username"}
            />
          )}
          ...
          <SelectField
            ...
            onChange={(e) => {
              ...
              dispatch({
                ...
                fields: v === "custom" ? { qrPosition: v } : { qrPosition: v, qrX: "", qrY: "" },
              });
            }}
          >
            {[
          ...
          {[
            ...
            <div key={key as string}>
              <div className="ctrl-row">
                <span className="ctrl-label">{lbl}</span>
                <span className="ctrl-val">
                  {form[key as keyof PosterForm] ? `${form[key as keyof PosterForm]}%` : `${def}%`}
                </span>
              </div>
              <input
                type="range"
                ...
                step="0.5"
                value={
                  form[key as keyof PosterForm] !== "" &&
                  form[key as keyof PosterForm] !== undefined
                    ? Number(form[key as keyof PosterForm])
                    : (def as number)
                }
                onChange={(e) => setXY(key as string, Number(e.target.value))}
              />
            </div>
          ))}
          <Slider
