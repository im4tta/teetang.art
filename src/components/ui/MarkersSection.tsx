...
import type { MarkerDefaults, MarkerItem } from "@/services/markers/types";
...
import {
  ...
  getUploadLabel,
} from "@/services/markers/helpers";
import { findMarkerIcon } from "@/services/markers/iconRegistry";
import {
  DEFAULT_MARKER_SIZE,
  MAX_MARKER_SIZE,
  MIN_MARKER_SIZE,
} from "@/services/markers/constants";
import { RADIUS_OPTIONS } from "@/services/config";
...
import { LogoUploadField as _LogoUploadField } from "@/components/ui/LogoUploadField";
      ...
      >
        <div className="marker-delete-modal__body">
          ...
          <p className="marker-delete-modal__text">
            {(t("markers.willRemove") as string).replace("{n}", String(count))}
          </p>
          <div className="marker-delete-modal__actions">
            <button type="button" className="marker-delete-modal__cancel" onClick={onCancel}>
              {t("markers.keepMarkers") as string}
            </button>
            <button type="button" className="marker-delete-modal__confirm" onClick={onConfirm}>
              <TrashIcon />
              {t("markers.deleteMarkers") as string}
            </button>
          </div>
        </div>
      </div>
    ...
    <div className="marker-color-control">
      <button type="button" className="marker-color-display-btn" onClick={onClick}>
        <span
          ...
          aria-hidden="true"
          style={{ backgroundColor: normalizeHexColor(color) || "#000000" }}
        />
...
export default function MarkersSection() {
  ...
  const { form, markers, customMarkerIcons, markerDefaults, isMarkerEditorActive, activeMarkerId } =
    state;
  ...
  const {
    locationSuggestions: ctSuggestions,
    isLocationSearching: ctSearching,
    clearLocationSuggestions: clearCtSuggestions,
    searchNow: searchCt,
  } = useLocationAutocomplete(customTextSearch, customTextFocused);
  ...
  useEffect(() => {
    const frame = window.requestAnimationFrame(() => {
      ...
      if (markers.some((marker) => marker.id === activeMarkerId)) setExpandedId(activeMarkerId);
      else {
        setExpandedId(null);
        setOpenColorPickerId(null);
      }
  ...
  const update = useCallback(
    (id: string, changes: Partial<MarkerItem>) =>
      dispatch({ type: "UPDATE_MARKER", markerId: id, changes }),
    [dispatch],
  );
  ...
  const applyDefaults = useCallback(
    (d: Partial<MarkerDefaults>) =>
      dispatch({ type: "SET_MARKER_DEFAULTS", defaults: d, applyToMarkers: true }),
    [dispatch],
  );
    ...
    (iconId: string) => {
      ...
      dispatch({
        ...
        marker: createMarkerItem({
          lat: (c?.lat ?? Number(form.latitude)) || 0,
          lon: (c?.lng ?? Number(form.longitude)) || 0,
          iconId,
  ...
  const openEditor = useCallback(
    (id: string) => {
      setExpandedId(id);
      setOpenColorPickerId(null);
      dispatch({ type: "SET_ACTIVE_MARKER", markerId: id });
    },
    [dispatch],
  );
  ...
  const handleAddCustomText = useCallback(() => {
    ...
    dispatch({
      type: "ADD_MARKER",
      marker: createMarkerItem({
        lat: selectedCustomLoc.lat,
        lon: selectedCustomLoc.lon,
        defaults: markerDefaults,
        label: customTextLabel.trim() || undefined,
      }),
    });
    setCustomTextSearch("");
    setCustomTextLabel("");
  ...
  const markerRows = useMemo(() => {
    ...
    return markers.map((marker, index) => {
      ...
      return {
        ...
        index,
        icon,
        markerLabel: `${lbl} ${counts.get(lbl)}`,
  ...
  const palette = useMemo(
    () =>
      DISPLAY_PALETTE_KEYS.map((k) =>
        getThemeColorByPath(effectiveTheme, k as ThemeColorKey),
      ).filter(Boolean),
    [effectiveTheme],
  );
  const defaultColorChoices = useMemo(
    () => buildDynamicColorChoices(markerDefaults.color, palette),
    [markerDefaults.color, palette],
  );

  const isColorFocused =
    isMarkerEditorActive && expandedId !== null && openColorPickerId === expandedId;
  const activeColorRow = isColorFocused
    ? (markerRows.find((r) => r.marker.id === expandedId) ?? null)
    : null;
  const activeColorChoices = activeColorRow
    ? buildDynamicColorChoices(activeColorRow.marker.color, palette)
    : null;
  const visibleRows =
    isMarkerEditorActive && expandedId
      ? markerRows.filter((r) => r.marker.id === expandedId)
      : markerRows;
        ...
        <div className="markers-section-head-actions">
          {!isMarkerEditorActive && (
            <button
              ...
              onClick={() =>
                setIsSettingsOpen((p) => {
                  if (p) setIsDefaultColorPickerOpen(false);
                  return !p;
                })
              }
              aria-label={
                isSettingsOpen
                  ? (t("markers.doneSettings") as string)
                  : (t("markers.openSettings") as string)
              }
            >
          ...
          {(isMarkerEditorActive || !isSettingsOpen) && (
            ...
            >
              ...
              <span className="marker-row__icon-btn-label">
                {isMarkerEditorActive
                  ? (t("markers.done") as string)
                  : (t("markers.editMarker") as string)}
              </span>
            </button>
          )}
          {!isMarkerEditorActive && !isSettingsOpen && (
            <button
              type="button"
              className={`marker-row__icon-btn marker-header-action-btn${isCustomTextOpen ? " is-active" : ""}`}
              onClick={toggleCustomText}
            >
              <span className="marker-row__icon-btn-icon" aria-hidden="true">
                {isCustomTextOpen ? <CloseIcon /> : <PlusIcon />}
              </span>
              <span className="marker-row__icon-btn-label">
                {isCustomTextOpen
                  ? (t("close") as string)
                  : (t("markers.addCustomTextMarker") as string)}
              </span>
            </button>
          )}
        </div>
      ...
      <div className="markers-section__content">
        {!isMarkerEditorActive && !isSettingsOpen && (
          <MarkerPicker
            ...
            onClearUploadedIcons={() => dispatch({ type: "CLEAR_CUSTOM_MARKER_ICONS" })}
          />
        )}
        ...
        {!isMarkerEditorActive && isCustomTextOpen && (
          <div className="marker-settings-card" style={{ marginTop: 12 }}>
            <div className="marker-settings-card__header">
              <h3>{t("markers.customTextMarker") as string}</h3>
            </div>
            <div className="marker-editor-card__stack" style={{ gap: 12 }}>
                ...
                <div className="location-search-row">
                  <div className="location-input-wrap" style={{ flex: 1 }}>
                    <input
                      ...
                      value={customTextSearch}
                      onChange={(e) => {
                        setCustomTextSearch(e.target.value);
                        setSelectedCustomLoc(null);
                      }}
                      onFocus={() => setCustomTextFocused(true)}
                      onBlur={() => setCustomTextFocused(false)}
                      onKeyDown={(e) => {
                        if (e.key === "Enter") void searchCt(e.currentTarget.value);
                      }}
                      placeholder={t("markers.searchLocation") as string}
                      autoComplete="off"
                    />
                    {customTextSearch && (
                      <button
                  ...
                  <button
                    ...
                    onMouseDown={(e) => e.preventDefault()}
                    onClick={() => searchCt(customTextSearch)}
                  >
                    <SearchIcon />
                  </button>
                </div>
                    ...
                    {ctSuggestions.map((s) => (
                      <li key={s.id}>
                        <button
                          ...
                          onMouseDown={(e) => {
                            e.preventDefault();
                            setSelectedCustomLoc(s);
                            setCustomTextSearch(s.label);
                            clearCtSuggestions();
                          }}
                        >
                          {s.label}
              ...
              <button
                ...
                disabled={!selectedCustomLoc}
              >
                <PlusIcon />
                {t("markers.addMarker") as string}
              </button>
            </div>
          </div>
        )}
        ...
        {!isMarkerEditorActive && isSettingsOpen && (
          <div className="marker-settings-card">
            <div className="marker-settings-card__header">
              ...
              <p className="marker-settings-card__theme-desc">
                {t("markers.settingsApply") as string}
              </p>
            </div>
            <div className="marker-editor-card__stack">
              <SliderWithInput
                label={t("markers.defaultSize") as string}
                value={markerDefaults.size}
                min={MIN_MARKER_SIZE}
                max={MAX_MARKER_SIZE}
                onChange={(v) => applyDefaults({ size: v })}
              />
              <div className="marker-settings-card__theme-color">
                <span className="marker-settings-card__theme-label">
                  {t("markers.defaultColor") as string}
                </span>
                <ColorButton
                  color={markerDefaults.color}
                  onClick={() => setIsDefaultColorPickerOpen((p) => !p)}
                />
              </div>
            </div>
            {isDefaultColorPickerOpen && (
              <ColorPicker
                ...
                onChange={(c) => applyDefaults({ color: c })}
                onResetColor={() => applyDefaults({ color: markerThemeColor })}
              />
            )}
          </div>
        )}
          ...
          <>
            {isColorFocused && activeColorRow ? (
              <article className="marker-editor-card">
                ...
                <ColorPicker
                  currentColor={activeColorRow.marker.color}
                  suggestedColors={activeColorChoices!.suggestedColors}
                  moreColors={activeColorChoices!.moreColors}
                  onChange={(c) => update(activeColorRow.marker.id, { color: c })}
                  onResetColor={() =>
                    update(activeColorRow.marker.id, { color: markerDefaults.color })
                  }
                />
              </article>
            ) : (
              <>
                {visibleRows.length > 0 ? (
                  <>
                    {!expandedId ? (
                      <div
                        className={
                          isMobile
                            ? "markers-section__mobile-strip"
                            : "markers-section__marker-grid"
                        }
                        role="list"
                      >
                          ...
                          <article key={marker.id} className="marker-editor-card">
                            <div className="marker-row">
                              <div className="marker-row__summary">
                                {icon && (
                                  <MarkerVisual icon={icon} size={26} color={marker.color} />
                                )}
                              ...
                              <div className="marker-row__actions">
                                <button
                                  type="button"
                                  className="marker-row__icon-btn"
                                  onClick={() => toggleEditor(marker.id)}
                                >
                                  <span className="marker-row__icon-btn-icon">
                                ...
                                <div className="field-grid keep-two-mobile">
                                  ...
                                  <label>
                                    ...
                                    <input
                                      ...
                                      min={-180}
                                      max={180}
                                      value={fmtCoord(marker.lon)}
                                      onChange={(e) => {
                                        const v = Number(e.target.value);
                                        if (Number.isFinite(v))
                                          update(marker.id, {
                                            lon: Math.max(-180, Math.min(180, v)),
                                          });
                                      }}
                                    />
                                  </label>
                                ...
                                <div className="marker-editor-card__stack">
                                  ...
                                  <div>
                                    ...
                                    <ColorButton
                                      color={marker.color}
                                      onClick={() =>
                                        setOpenColorPickerId((p) =>
                                          p === marker.id ? null : marker.id,
                                        )
                                      }
                                    />
                                  </div>
                  ...
                  <p className="marker-settings-card__theme-note">
                    {t("markers.addMarkerNote") as string}
                  </p>
                )}
                  ...
                  <>
                    ...
                    <div className="markers-section__actions">
                      ...
                      >
                        <RotateLeftIcon />
                        <span className="marker-row__icon-btn-label">
                          {t("markers.resetMarkers") as string}
                        </span>
                      </button>
                      <button
                        type="button"
                        className="marker-row__icon-btn marker-row__icon-btn--danger"
                        onClick={() => hasMarkers && setDeleteModalOpen(true)}
                      >
                        <TrashIcon />
                        <span className="marker-row__icon-btn-label">
                          {t("markers.deleteAllMarkers") as string}
                        </span>
                      </button>
                    </div>
                  </>
