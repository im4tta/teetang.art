import { MIN_DISTANCE_METERS, MAX_DISTANCE_METERS } from "@/services/config";
...
export default function MapDimensionFields({
  ...
  const distanceToSliderValue = (distanceMeters: number) => {
    ...
    const ratio = (Math.log(clampDistance(distanceMeters)) - minLog) / (maxLog - minLog);
  ...
  const sliderMetaMarks = [MIN_DISTANCE_METERS, 100_000, 1_000_000, MAX_DISTANCE_METERS];

  const sliderPositionPercent = (distanceMeters: number) =>
    ((distanceToSliderValue(distanceMeters) - SLIDER_MIN) / (SLIDER_MAX - SLIDER_MIN)) * 100;
      ...
      {showDistanceField ? (
        <label className="distance-slider-field">
          ...
          <input
            ...
            value={sliderValue}
            onChange={(event) =>
              emitDistanceChange(sliderValueToDistance(Number(event.target.value)))
            }
            aria-label="Distance in meters"
          />
