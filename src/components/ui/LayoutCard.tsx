...
import { formatLayoutDimensions } from "@/services/layout/layoutRepository";
...
export default function LayoutCard({ layoutOption, onClick, isSelected = false }: LayoutCardProps) {
  if (!layoutOption) {
    return null;
  }
  const className = ["layout-card", isSelected ? "is-selected" : ""].filter(Boolean).join(" ");
