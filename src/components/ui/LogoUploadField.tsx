...
export function LogoUploadField({ form }: { form: PosterForm }) {
  ...
  const setField = (fields: Record<string, string>) =>
    dispatch({ type: "SET_FORM_FIELDS", fields });

  return (
    <>
      <div className="ios-toggle-row">
        <span className="ios-toggle-label">{t("logo.show")}</span>
        <label className="ios-toggle">
          <input
            ...
            onChange={(e) => {
              ...
              if (!e.target.checked) dispatch({ type: "SET_FIELD", name: "logoUrl", value: "" });
            }}
          />
      ...
      {show && (
        <>
          ...
          >
            {t(form.logoUrl ? "logo.change" : "logo.upload")}
          </button>
          {form.logoUrl && (
            <img
              src={form.logoUrl}
              alt="Logo preview"
              style={{ maxWidth: 120, maxHeight: 60, marginTop: 8, display: "block" }}
            />
          )}
          {form.logoUrl && (
            <>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setField(
                        v === "custom"
                          ? { logoPosition: v }
                          : { logoPosition: v, logoX: "", logoY: "" },
                      );
                    }}
                  >
                    {POSITION_OPTIONS.map(([v, k]) => (
                ...
                <div key={key as string}>
                  <div className="ctrl-row">
                    ...
                    <span className="ctrl-val">
                      {form[key as string] !== "" && form[key as string] !== undefined
                        ? `${form[key as string]}%`
                        : `${def}%`}
                    </span>
                  </div>
                  <input
                    ...
                    value={
                      form[key as string] !== "" && form[key as string] !== undefined
                        ? Number(form[key as string])
                        : def
                    }
                    onChange={(e) =>
                      setField({
                        [key as string]: String(Number(e.target.value)),
                        logoPosition: "custom",
                      })
                    }
                  />
              ...
              <label style={{ marginTop: 12 }}>
                ...
                <input
                  ...
                  value={form.logoPadding}
                  onChange={(e) => setField({ logoPadding: e.target.value })}
                />
              </label>
            </>
          )}
