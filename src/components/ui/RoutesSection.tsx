  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      {(searching || suggestions.length > 0) && (
        ...
        >
          {searching ? (
            <div className="location-suggestions__item">{t("location.searching")}</div>
          ) : (
            suggestions.map((s) => (
              <button
                key={`${s.lat}-${s.lon}`}
      ...
      >
        ...
        <span className="route-card__meta">
          <span className="route-card__label">
            {editingEP
              ? t("routes.editPoint").replace(
                  "{point}",
                  t(activeEP === "start" ? "routes.start" : "routes.finish"),
                )
              : route.label}
          </span>
      ...
      {isOpen && !editingEP && (
        <div className="route-card__body">
          <label className="toggle-field">
            <span>{t("routes.showEndpoints")}</span>
                ...
                return (
                  ...
                  >
                    ...
                    <span className="route-card__endpoint-row-label">
                      {t(kind === "start" ? "routes.start" : "routes.finish")}
                    </span>
                  </button>
                );
              })}
          ...
          <div className="route-card__field">
            ...
            <div className="route-card__style-toggle">
              {ROUTE_LINE_STYLES.map((style) => (
                ...
                >
                  <span>{t(style === "solid" ? "routes.solid" : "routes.dashed")}</span>
                  <svg
          ...
          <div className="route-card__actions">
            ...
            >
              <TrashIcon />
              <span className="marker-row__icon-btn-label">{t("routes.remove")}</span>
            </button>
          </div>
        </div>
      )}
  ...
  return (
    <section className="panel-block routes-settings-screen">
      <div className="markers-section-head">
        <p className="section-summary-label">{t("nav.routes")}</p>
        <div className="markers-section-head-actions">
          ...
          >
            <span className="marker-row__icon-btn-label">
              {t(form.showRoutes ? "routes.hideAll" : "routes.showAll")}
            </span>
          </button>
          <button
            type="button"
          ...
          >
            <span className="marker-row__icon-btn-label">
              {t(state.routeDrawMode ? "ui.cancel" : "routes.draw")}
            </span>
          </button>
        </div>
      </div>
      ...
      <div className="ios-toggle-row" style={{ marginBottom: 8 }}>
        <span className="ios-toggle-label">{t("routes.snapToRoads")}</span>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 12 }}>
        ...
        <button
          ...
          style={{
            ...
            opacity: !routeA.loc || !routeB.loc ? 0.5 : 1,
          }}
        >
      ...
      <div className="routes-section__content">
        {!routes.length && <p className="routes-section__empty">{t("routes.emptyHint")}</p>}
        {routes.map((route) => (
          <RouteCard
            key={route.id}
            route={route}
