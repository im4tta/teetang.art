...
function LocationInput({
  ...
  // The blur is deferred so a suggestion click still lands, but the timer has
  // to be cancelled on unmount or it calls setState on a dead component.
  useEffect(
    () => () => {
  ...
  const showSuggestions = focused && (searching || suggestions.length > 0);
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      >
        <label htmlFor={inputId} style={{ fontSize: 11, color: "#94a3b8" }}>
          {label}
        </label>
        <button
          type="button"
      ...
      {showSuggestions && (
        ...
        >
          {searching ? (
            <div className="location-suggestions__item">Searching…</div>
          ) : (
            suggestions.map((s) => (
              <button
                key={`${s.lat}-${s.lon}`}
              ...
              >
                {s.label}
                {s.city && (
                  <span className="location-suggestions__sub">
                    {s.city}, {s.country}
                  </span>
                )}
              </button>
...
function RouteCard({
  ...
  return (
    <article className={`route-card${isOpen ? " is-open" : ""}`}>
      {!isOpen ? (
        <button type="button" className="route-card__delete" onClick={() => onRemove(route.id)}>
          <CloseIcon />
        </button>
      ) : (
        <button
      ...
      {isOpen && !editingEP && (
        <div className="route-card__body">
          <label className="toggle-field">
            ...
            <span className="theme-switch">
              <input
                ...
                onChange={(e) => onUpdate(route.id, { showEndpoints: e.target.checked })}
              />
              ...
              {(["start", "finish"] as const).map((kind) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => setActiveEP(kind)}
                  >
                    <span className="route-card__endpoint-row-icon">
                      {icon && <MarkerVisual icon={icon} size={24} color={ep.color} />}
                    </span>
          ...
          <div className="route-card__field">
            <label htmlFor={`rw-${route.id}`}>
              <span>Width</span>
              <span className="route-card__value">{route.strokeWidth}px</span>
            </label>
            <input
              id={`rw-${route.id}`}
              type="range"
              ...
              onChange={(e) => onUpdate(route.id, { strokeWidth: Number(e.target.value) })}
            />
          </div>

          <div className="route-card__field">
            <label htmlFor={`ro-${route.id}`}>
              <span>Opacity</span>
              <span className="route-card__value">{Math.round(route.opacity * 100)}%</span>
            </label>
            <input
              id={`ro-${route.id}`}
              type="range"
              ...
              step={0.05}
              value={route.opacity}
              onChange={(e) => onUpdate(route.id, { opacity: Number(e.target.value) })}
            />
          </div>
            ...
            <div className="route-card__style-toggle">
              {ROUTE_LINE_STYLES.map((style) => (
                <button
                  ...
                  onClick={() => onUpdate(route.id, { lineStyle: style })}
                >
          ...
          <div className="route-card__actions">
            ...
            >
              <TrashIcon />
              <span className="marker-row__icon-btn-label">Remove Route</span>
            </button>
          </div>
        </div>
      )}
...
export default function RoutesSection() {
  ...
  const updateEP = useCallback(
    (routeId: string, kind: Kind, changes: Partial<RouteEndpointMarker>) => {
      ...
      dispatch({
        ...
        changes:
          kind === "start"
            ? { startMarker: { ...current, ...changes } }
            : { finishMarker: { ...current, ...changes } },
      });
    },
    [dispatch, routes],
  );
      ...
      navigator.geolocation.getCurrentPosition(
        ({ coords: { latitude: lat, longitude: lon } }) => {
          const loc: SearchResult = {
            id: `cur:${lat},${lon}`,
            label: t("routes.useCurrentLocation"),
            shortLabel: t("routes.useCurrentLocation"),
            city: "",
            country: "",
            continent: "",
            lat,
  ...
  const createRoute = useCallback(
    async (a: SearchResult, b: SearchResult) => {
      ...
      try {
        ...
        const route: Route = {
          id,
          label: `${a.city || a.label} → ${b.city || b.label}`,
          color: state.routeDefaults.color || "#3b82f6",
          opacity: state.routeDefaults.opacity ?? 0.8,
          strokeWidth: state.routeDefaults.strokeWidth ?? 4,
          lineStyle: "solid",
          source: "named",
          visible: true,
          showEndpoints: true,
        ...
        // Keep a local copy of the final geometry so the recenter below uses the
        // snapped path rather than the straight A→B line.
        ...
        if (form.snapToRoads) {
          ...
          if (coords && coords.length > 1) {
            finalRoute = { ...route, segments: [coords] };
            dispatch({ type: "UPDATE_ROUTE", routeId: id, changes: { segments: [coords] } });
          }
        }
        ...
        const bounds = routeBounds(finalRoute);
        if (bounds) {
    ...
    <section className="panel-block routes-settings-screen">
      <div className="markers-section-head">
        ...
        <div className="markers-section-head-actions">
          <button
            ...
            onClick={() =>
              dispatch({ type: "SET_FIELD", name: "showRoutes", value: !form.showRoutes })
            }
          ...
          >
            <span className="marker-row__icon-btn-label">
              {state.routeDrawMode ? "Cancel" : "Draw Route"}
            </span>
          </button>
        </div>
      </div>
      ...
      <div className="ios-toggle-row" style={{ marginBottom: 8 }}>
        ...
        <label className="ios-toggle">
          <input
            type="checkbox"
            checked={Boolean(form.snapToRoads)}
            onChange={() =>
              dispatch({ type: "SET_FIELD", name: "snapToRoads", value: !form.snapToRoads })
            }
          />
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 12 }}>
        <LocationInput
          ...
          onSelect={(s) => {
            setRouteA({ input: s.label, loc: s, focused: false });
            autoA.clearLocationSuggestions();
          }}
        ...
        <LocationInput
          ...
          onSelect={(s) => {
            setRouteB({ input: s.label, loc: s, focused: false });
            autoB.clearLocationSuggestions();
          }}
        ...
        <button
          ...
          disabled={!routeA.loc || !routeB.loc || creatingRoute}
          aria-busy={creatingRoute}
          onClick={() => {
            if (routeA.loc && routeB.loc) void createRoute(routeA.loc, routeB.loc);
          }}
          style={{
            width: "100%",
            justifyContent: "center",
            opacity: !routeA.loc || !routeB.loc || creatingRoute ? 0.5 : 1,
          }}
        >
          {creatingRoute ? "Creating…" : t("routes.createRoute")}
        </button>
      </div>
      ...
      <div className="routes-section__content">
        {!routes.length && (
          <p className="routes-section__empty">
            Press <strong>Draw Route</strong> above, then click two points on the map.
          </p>
        )}
        {routes.map((route) => (
          <RouteCard
