...
import type { MarkerDefaults, MarkerIconDefinition, MarkerItem } from "@/services/markers/types";
...
export function posterReducer(state: PosterState, action: PosterAction): PosterState {
  switch (action.type) {
    ...
    case "SET_MARKER_DEFAULTS": {
      ...
      return {
        ...
        markers: action.applyToMarkers
          ? state.markers.map((m) => ({
              ...m,
              ...(hasSize ? { size: next.size } : {}),
              ...(hasColor ? { color: next.color } : {}),
            }))
          : state.markers,
      };
    }
    ...
    case "RESET_DESIGN":
      // Start over, keeping the app preferences and the uploaded icon library.
      return {
        ...INITIAL_STATE,
        ...
        form: {
          ...INITIAL_STATE.form,
          appTheme: state.form.appTheme,
          uiDensity: state.form.uiDensity,
        },
      };
    default:
      return state;
